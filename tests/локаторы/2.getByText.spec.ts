...
test.describe('Сложные случаи поиска по тексту', () => {
  ...
  test('Найти вложенный текст', async ({ page }) => {
    const nestedSpan = page.locator('p span', { hasText: 'вложенным текстом' }); // TODO(student): замените на корректный локатор
  ...
  test('Работа с динамическим контентом', async ({ page }) => {
    const dynamicText = page.getByText('Динамически загруженный текст Этот текст появился после загрузки страницы'); // TODO(student): замените на корректный локатор
  ...
  test('Найти текст с пробелами', async ({ page }) => {
    const spacedText = page
      .getByText(/\s{2,}/)
      .and(page.locator('section.text-section', { hasText: 'Сложные случаи' }));
