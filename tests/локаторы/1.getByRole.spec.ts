...
test.describe('Поиск элементов по роли "button"', () => {
  ...
  test('Найти основную кнопку по роли и тексту', async ({ page }) => {
    const primaryButton = page.getByRole('button', { name: 'Основное действие' }); // TODO(student): замените на корректный локатор - OK
  ...
  test('Найти неактивную кнопку по роли и состоянию', async ({ page }) => {
    const disabledButton = page.getByRole('button', { name: 'Неактивная кнопка' }); // TODO(student): замените на корректный локатор - ОК
  ...
  test('Найти div с ролью кнопки', async ({ page }) => {
    const divButton = page.getByRole('button', { name: 'Div как кнопка' }); // TODO(student): замените на корректный локатор - ОК
...
test.describe('Поиск элементов форм по ролям', () => {
  ...
  test('Найти поля формы по их ролям', async ({ page }) => {
    const usernameInput = page.getByRole('textbox', { name: 'Имя пользователя' }); // TODO(student): замените на корректный локатор - ОК
  ...
  test('Найти чекбоксы по роли checkbox', async ({ page }) => {
    const newsletterCheckbox = page.getByRole('checkbox', { name: 'Подписаться на рассылку' }); // TODO(student): замените на корректный локатор - ОК
...
const countryInput = page.getByLabel('Страна');
await expect(countryInput).toBeVisible();
await countryInput.selectOption('de');
await expect(countryInput).toHaveValue('de');

    // 4. Найди и нажми кнопку отправки

    const confirmButton = page.getByRole('button', { name: 'Отправить' })
    await expect(confirmButton).toBeVisible();
  await expect(confirmButton).toHaveClass(/primary-btn/);
